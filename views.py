...
from .models import Profile,Strength
import os
import datetime
import pandas as pd
...
def index(request):
    ...
    if request.user.is_authenticated:
        try:
            ...
            m=Profile.objects.all()
        except:
            img=""
        return render(request,'website/home.html',{'image':img,"m":m})
    else:
...
def buy(request):
    ...
    filename=r"D:\\final year project\\food recapp\\minor\\website\\csvfile\\recent_activity.csv"
    ...
    for meal_id in l:
        ...
        df2=df2.concat(pd.Series(lst,index=df2.columns),ignore_index=True)
...
def order(request):
    if Profile.objects.get(number=request.user.username).second_time:
        filename=r"D:\\final year project\\food recapp\\minor\\website\\csvfile\\recent_activity.csv"
        filename2=r"D:\\final year project\\food recapp\\minor\\website\\dataset.csv"
        ...
        for meal in l:
            ...
            if data.empty :
                ...
                data=data.concat(pd.Series(lst[0],index=df2.columns),ignore_index=True)
...
def LikeRate(request):
    if request.method=='POST':
        ...
        for meal in ids:
            ...
            df=df.concat(pd.Series(lst,index=df.columns),ignore_index=True) #lst = values in list which we want to insert
            os.remove(filename)
...
def strength(request):
    ...
    return render(request,'website/strength.html',{"strength":strength})

def calc(request,pk):
    strength=Strength.objects.get(id=pk)
    if request.method=='POST':
        ...
        print("hi")
        if weight is not None and lift is not None and age is not None and reps is not None:
            weight = float(weight)
            lift = float(lift)
            age = int(age)
            reps = int(reps)
            one_rep_max =  lift * (1 + (reps / 30)) 
            print(one_rep_max)
            beginner = float(strength.Beginner)
            novice = float(strength.Novice)
            intermediate = float(strength.Intermediate)
            advanced = float(strength.Advanced)

            if one_rep_max <= beginner:
                typ = "Beginner"
            elif one_rep_max <= novice:
                typ = "Novice"
            elif one_rep_max <= intermediate:
                typ = "Intermediate"
            elif one_rep_max <= advanced:
                typ = "Advanced"
            else:
                typ = "Elite"
            return render(request, 'website/calc.html', {'typ': typ,"strength":strength,"one":one_rep_max})
    return render(request,'website/calc.html',{"strength":strength})
